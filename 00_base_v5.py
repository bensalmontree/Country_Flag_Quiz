...
class ChooseRounds:
    ...
    def __init__(self):
        ...
        # Insert temp instruction text 
        self.intro_entry.insert(0, "Enter number of rounds")    
        self.intro_entry.bind("<FocusIn>", self.temp_text)

        # Go button
        self.confirm = Button(self.intro_frame, text="Go", width=7, bg="#90EE90", command=lambda: self.num_check())
    ...
    # Delete temp text in entry box when clicked
...
class Play:
    ...
    def __init__(self, how_many):
        ...
        # If users press cross at top, verify if user wants to quit
        self.quiz_box.protocol('WM_DELETE_WINDOW', self.on_closing)
        ...
        self.score_heading = Label(self.heading_frame, text=self.score, font=("Arial", "12", "bold"), justify=LEFT, anchor=W)
        ...
        # get countries for buttons for first round ...
        self.button_countries_list = []

        for item in range (0, 4):
            
            self.choice_button = Button(self.choice_frame, width=20, height=2, text="", command=lambda i=item: self.to_compare(self.button_countries_list[i], i))
    ...
    def get_round_flag(self):    
        ...
        # Generate correct answer and display flag
    ...
    def to_compare(self, user_choice, button_num):
        ...
        if user_choice == self.correct_ans:
            ...
            self.img = Image.open("Country_Flag_Quiz\happy.png")
            
            # Add one to number of answers correct for score
            current_score = self.rounds_won.get()
            current_score += 1
            self.rounds_won.set(current_score)
        ...
        # Otherwise find the correct answer
            ...
            self.img = Image.open("Country_Flag_Quiz\sad.png")
        ...
        # After choice, display image of emoji
        Emoji(self.img)
        
        # Disable choice buttons after choice
        count = 0
        for item in self.choice_button_ref:
            item['state'] = DISABLED
        ...
        if current_round == how_many:
            self.next_button.config(text="End Game", command=lambda: self.to_endscreen())
        ...
        # update score (AFTER button press)
        self.amount_correct = self.rounds_won.get()
        amount_played = self.rounds_played.get()
        self.score_heading.config(text="Score: {} / {}".format(self.amount_correct, amount_played))
    
    # Verify on closing program 
    def on_closing(self):
        if messagebox.askokcancel("Quit", "Do you want to quit?"):
            
            # Go to endscreen / show score
            self.to_endscreen()
        
    # Go to endscreen after done playing
    def to_endscreen(self):
        
        EndScreen(self.score)
...
class EndScreen:
    ...
    def __init__(self, score):
        ...
        self.end_screen_box.geometry("200x180")
        
        # Frame to hold text
        self.text_frame = Frame(self.end_screen_box, padx=10, pady=10)
        self.text_frame.grid()
        
        # Text showing score
        self.score = Label(self.text_frame, text=score, font=("Arial", "16", "bold"), justify=CENTER, anchor=W)
        self.score.grid(row=0, sticky=W)
        
        # Button to play again
        self.play_again_button = Button(self.end_screen_box, text= "Play Again?", command=lambda: self.we_go_again(), width=20, height=5)
        self.play_again_button.grid(padx=20, pady=10, row=1)
        
    # If 'Play Again?' is pressed, restart program
...
class Emoji:
    ...
    def __init__(self, img):
        ...
        self.emoji_box = Toplevel()
        
        # Frame to hold image of emoji
        self.img_frame = Frame(self.emoji_box, padx=10, pady=10)
        self.img_frame.grid()
        
        ph = ImageTk.PhotoImage(img)
        
        img = Label(self.img_frame)
        img.grid()
        ...
        # Close window after sufficient time
        self.emoji_box.after(800,lambda:self.emoji_box.destroy())
